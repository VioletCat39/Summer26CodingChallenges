...
def decode(cipher, key):
    """ decodes the secret message given the encrypted message and key """
    decoded_message = ""
    for letter in cipher:
        decoded_message += shift(letter, 26-key)

    return decoded_message

def main():
    """ Given a message and a key, this will encode and decode the message """
    message = str(input("Please enter the message to be encoded: "))
    key = int(input("Please enter the key to encrypt this message: "))
    print("Here is the encoded message: " + encode(message,key))
    #secret_message = encode(message,key)
    #print("Here is the decoded message: " + decode(secret_message, key))
